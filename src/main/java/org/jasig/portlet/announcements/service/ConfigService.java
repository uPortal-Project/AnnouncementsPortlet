...
public class ConfigService implements IConfigService {
  ...
  public AnnouncementConfiguration getConfiguration(PortletRequest request) {
    ...
    config.setFilterType(
        prefs.getValue(
            AnnouncementPreferences.FILTER_TYPE.getKey(),
            AnnouncementFilterType.BLACKLIST.getKey()));
    config.setFilterItems(
        prefs.getValues(AnnouncementPreferences.FILTER_ITEMS.getKey(), new String[0]));
    return config;
  }
  ...
  public void saveConfiguration(ActionRequest request, AnnouncementConfiguration config) {
    ...
    try {
      ...
      prefs.setValues(
          ...
          config.getFilterItems().toArray(new String[0]));
